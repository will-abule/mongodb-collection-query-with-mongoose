function searchFilter(io, reqQuery) {
  if (reqQuery.searchFilters === undefined)
    return (() => {
      ...
      if (
        ...
        return (() => {
          for (const client of reqQuery.client) {
            io.to(client.id).emit("error", {
              status: 400,
              message: "rules field is required and must be set to 'string'",
            });
          }
        })();
      }
        ...
        return (() => {
          for (const client of reqQuery.client) {
            io.to(client.id).emit("error", {
              status: 400,
              message:
                "rules type is set to 'string' rules but data is not a string",
            });
          }
        })();
        ...
        return (() => {
          for (const client of reqQuery.client) {
            io.to(client.id).emit("error", {
              status: 400,
              message:
                "rules type is set to 'number' rules but data is not a number",
            });
          }
        })();
      } else if (
        ...
        return (() => {
          for (const client of reqQuery.client) {
            io.to(client.id).emit("error", {
              status: 400,
              message:
                "rules type is set to 'float' rules but data is not a float",
            });
          }
        })();
        ...
        return (() => {
          for (const client of reqQuery.client) {
            io.to(client.id).emit("error", {
              status: 400,
              message:
                "rules type is set to 'date' rules but data is not a date",
            });
          }
        })();
        ...
        return (() => {
          for (const client of reqQuery.client) {
            io.to(client.id).emit("error", {
              status: 400,
              message:
                "rules type is set to 'range' rules but data is not a array",
            });
          }
        })();
      } else if (
      ...
      if (data.option === "in" && !Array.isArray(data.data)) {
        return (() => {
          for (const client of reqQuery.client) {
            io.to(client.id).emit("error", {
              status: 400,
              message: "rules option must is set to 'in' but is not an array",
            });
          }
        })();
      ...
      if (data.option === "nin" && !Array.isArray(data.data)) {
        return (() => {
          for (const client of reqQuery.client) {
            io.to(client.id).emit("error", {
              status: 400,
              message: "rules option must is set to 'nin' but is not an array",
            });
          }
        })();
  ...
  if (searchFilter.searchOption !== "OR" && searchFilter.searchOption !== "AND")
    return (() => {
      for (const client of reqQuery.client) {
        io.to(client.id).emit("error", {
          status: 400,
          message: "searchOption should either 'AND' or 'OR' in uppercase",
        });
      }
    })();
}
